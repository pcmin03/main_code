...
class mydataset_3d(Dataset):
    ...
    dataname,phase):
        ...
        for i in tqdm(range(len(self.imageDir))):
            ...
            if 'uint16_wise' in self.dataname:
        ...
        self.L_transform =  transforms.Compose([
                        ...
                        custom_transforms.Normalize_3d((mean),(std))])
    ...
    def __getitem__(self,index):
        ...
        _mask = np.where(image > 0.5,np.zeros_like(image),np.ones_like(image))[np.newaxis]
        
        # print(_mask.max(),_mask.min(),_mask.shape)
        # self.L_transform(
            
        clip = self.L_transform(image)
        # print(clip.shape,clip.max(),clip.min())
