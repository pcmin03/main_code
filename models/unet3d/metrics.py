...
from .utils import get_logger, adapted_rand, expand_as_one_hot, plot_segm
...
class MeanIoU:
    ...
    def __call__(self, input, target):
        ...
        for _input, _target in zip(input, target):
            ...
            mean_iou += torch.tensor(per_channel_iou)
    ...
    # def precision_index(self,prediction, target):
    #     Class_precision = torch.sum(prediction & target).float() / torch.sum(prediction & target).float()
        
    # def recall_index(self,prediction, target):
    #     Class_recall = torch.sum(prediction & target).float() / np.sum(self.confusion_matrix, axis=1)
        
    # def f1_index(self,prediction, target):
