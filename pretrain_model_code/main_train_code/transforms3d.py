...
class RandomFlip:
    ...
    def __init__(self, random_state, axis_prob=0.5):
...
class RandomRotate:
    ...
    def __call__(self, m,n):
        ...
        if m.ndim == 3:
            ...
            n = rotate(n, angle, axes=axis, reshape=False, order=self.order, mode=self.mode, cval=-1)
        else:
            channels = [rotate(m[c], angle, axes=axis, reshape=False, order=self.order, mode=self.mode, cval=-1) for c
                        in range(m.shape[0])]
            m = np.stack(channels, axis=0)
            channels = [rotate(n[c], angle, axes=axis, reshape=False, order=self.order, mode=self.mode, cval=-1) for c
                                    in range(n.shape[0])]
            n = np.stack(channels, axis=0)
...
class RandomContrast:
    ...
    def __init__(self, random_state, alpha=(0.5, 1.5), mean=0.0, execution_probability=0.1):
        self.random_state = random_state
        assert len(alpha) == 2
        self.alpha = alpha
        self.mean = mean
...
class ToTensor:
    ...
    def __init__(self, expand_dims=True, dtype=np.uint8, **kwargs):
